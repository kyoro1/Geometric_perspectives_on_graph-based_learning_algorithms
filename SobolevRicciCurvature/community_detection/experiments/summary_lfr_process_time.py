...
csv_path = "./results/lfr_orc_src.csv"
df = pd.read_csv(csv_path)

agg = (
    df.groupby(["mu", "method"])["time_flow_sec"]
      .agg(["mean", "std", "count"])
      .reset_index()
)
...
for i, method in enumerate(methods):
    sub = agg[agg["method"] == method].set_index("mu").reindex(mus)
...
plt.tight_layout()
plt.savefig("./results/summary_lfr_process_time.pdf", dpi=300)