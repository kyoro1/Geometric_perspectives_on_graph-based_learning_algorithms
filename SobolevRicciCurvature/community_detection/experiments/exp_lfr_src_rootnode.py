...
def main():
    ...
    src_rootnodes = np.random.randint(0, args.lfr_nodes, size=args.trials)

    tasks = list(product(methods, mus, range(args.trials), src_rootnodes))
    ...
    rows: List[Dict[str, Any]] = Parallel(n_jobs=-1, backend="loky", verbose=10)(
        ...
        for method, mu, trial, src_root in tasks
    )
