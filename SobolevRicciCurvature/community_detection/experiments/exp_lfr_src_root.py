...
def run(lfr_nodes: int,
    ...
    method_obj = make_method(method, 
                             steps, 
                             alpha, 
                             method_kwargs=method_kwargs,        # SRC専用
                             )
...
def main():
    ...
    ap.add_argument("--src_roots", type=int, default=0, help="SRC source root node (integer index)")
    ## For ORC params
    ...
    tasks = list(product(methods, mus, range(args.src_roots)))
    ...
    rows: List[Dict[str, Any]] = Parallel(n_jobs=-1, backend="loky", verbose=10)(
        delayed(run)(
            ...
            method_kwargs={"p": args.src_p,
                          "src_root": src_root,  ## Use trial number as src_root for reproducibility
                           "orc_calc": args.orc_calc,
                           "orc_eta": args.orc_eta,
                           },
        )
        for method, mu, src_root in tasks
    )
