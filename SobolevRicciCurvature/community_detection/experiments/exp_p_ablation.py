...
def extract_edge_lengths(G, length_attr="length"):
    edges = sorted(G.edges())
    L = np.array([float(G[u][v].get(length_attr, 1.0)) for u, v in edges], dtype=np.float64)
    U = np.array([u for u, v in edges], dtype=np.int32)
    V = np.array([v for u, v in edges], dtype=np.int32)
    return U, V, L
...
def run(lfr_nodes: int,
    ...
    # --- inside run() right after Gf is computed ---
    save_dir = method_kwargs.get("save_dir", "./histories")
    os.makedirs(save_dir, exist_ok=True)

    U, V, L_final = extract_edge_lengths(Gf, length_attr="length")

    out_npz = os.path.join(
        save_dir,
        f"final_{method}_mu{mu:.2f}_p{method_kwargs['p']:.1f}_seed{seed}.npz"
    )
    ...
    print("[save] final lengths saved:", out_npz, flush=True)
...
def main():
    ...
    tasks = list(product(methods, mus, src_ps, range(args.trials)))
    ...
    rows: List[Dict[str, Any]] = Parallel(n_jobs=-1, backend="loky", verbose=10)(
        ...
        for method, mu, src_p, trial in tasks
    )
